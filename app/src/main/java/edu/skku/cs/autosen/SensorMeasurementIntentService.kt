...
class SensorMeasurementIntentService : IntentService("SensorMeasurementIntentService") {
    ...
    override fun onHandleIntent(intent: Intent?) {
        ...
        sensorManager.registerListener(acceleroLis, acceleroSensor, SensorManager.SENSOR_DELAY_UI)
        sensorManager.registerListener(gyroLis, gyroSensor, SensorManager.SENSOR_DELAY_UI)
        sensorManager.registerListener(magneticLis, magneticSensor, SensorManager.SENSOR_DELAY_UI)
    ...
    inner class AccelerometerListener : SensorEventListener {
        ...
        override fun onSensorChanged(p0: SensorEvent?) {
            if (p0!!.sensor.type == Sensor.TYPE_ACCELEROMETER) {
                ...
                //Log.d("asdf","${timeFormat.format(System.currentTimeMillis())} accX: ${p0!!.values[0]} accY: ${p0!!.values[1]} accZ: ${p0!!.values[2]}\\n")
            }
        }
    }
